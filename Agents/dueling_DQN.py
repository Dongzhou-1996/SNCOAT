...
from Models.critic_models import DuelingConvNet, DuelingResNet, BasicBlock, Bottleneck
...
class DuelingDQN(object):
    ...
    def __init__(self, name='vanilla_dqn', log_dir='log', observe_type='Color', backbone='ConvNet',
        ...
        if self.backbone == 'ConvNet':
            self.eval_net = DuelingConvNet(observe_type=observe_type, action_dim=self.action_dim).to(self.device)
            self.target_net = DuelingConvNet(observe_type=observe_type, action_dim=self.action_dim).to(self.device)
        elif self.backbone == 'ResNet18':
            self.eval_net = DuelingResNet(observe_type=observe_type, action_dim=self.action_dim,
                                          block=BasicBlock, num_blocks=[2, 2, 2, 2]).to(self.device)
            self.target_net = DuelingResNet(observe_type=observe_type, action_dim=self.action_dim,
                                            block=BasicBlock, num_blocks=[2, 2, 2, 2]).to(self.device)
    ...
    def learn(self):
        ...
        batch_states, batch_action_idxs, batch_rewards, batch_next_states, batch_dones = self.memory.sample(
            self.batch_size)
