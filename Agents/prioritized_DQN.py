...
from Utils.prioritized_replay_memory import PrioritizedMemory, Transition
from Envs.SNCOAT_Env_v1 import DISCRETE_ACTIONS


class PrioritizedDQN(object):
    """Prioritized DQN algorithm"""

    def __init__(self, name='prioritized_dqn', log_dir='log', observe_type='Color', backbone='ConvNet',
                 action_type='Discrete', actuator_type='Position', action_dim=11, gpu_idx=0,
                 replay_buffer_size=50000, batch_size=32, lr=1e-4,
                 gamma=0.99, start_epsilon=0.9, end_epsilon=0.1,update_interval=1000,
                 restore=False, is_training=True):
        ...
        assert backbone in ['ConvNet', 'ResNet18', 'ResNet34', 'ResNet50'], print('Unsupported backbone type!')
        ...
        if self.backbone == 'ConvNet':
            ...
            self.eval_net = ResQNet(observe_type=observe_type, action_dim=self.action_dim,
                                    block=BasicBlock, num_blocks=[2, 2, 2, 2]).to(self.device)
            self.target_net = ResQNet(observe_type=observe_type, action_dim=self.action_dim,
                                      block=BasicBlock, num_blocks=[2, 2, 2, 2]).to(self.device)
            ...
            self.eval_net = ResQNet(observe_type=observe_type, action_dim=self.action_dim,
                                    block=Bottleneck, num_blocks=[3, 4, 6, 3]).to(self.device)
            self.target_net = ResQNet(observe_type=observe_type, action_dim=self.action_dim,
                                      block=Bottleneck, num_blocks=[3, 4, 6, 3]).to(self.device)
        else:
        ...
        self.memory = PrioritizedMemory(capacity=self.replay_buffer_size, prob_alpha=0.4)
    ...
    def learn(self):
        ...
        beta = self.start_beta * np.exp(0.001 * self.episode_counter)
        beta = min(1.0, beta)
        self.summary_writer.add_scalar('beta_factor', beta, self.episode_counter)
        batch_states, batch_action_idxs, batch_rewards, batch_next_states, batch_dones, batch_indices, batch_weights = self.memory.sample(self.batch_size, beta)
        ...
        loss = self.loss_func(q_eval, q_target.to(self.device)) * batch_weights
        priors = loss + 1e-5
        loss = loss.mean()
        ...
        loss.backward()
        self.optimizer.step()
        self.memory.update_priorities(batch_indices, priors.data.cpu().numpy())
